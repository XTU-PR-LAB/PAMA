...
def prepare_env(args):
    ...
    sub_dir = 'pama_' + cfg.MODEL.BACKBONE
    if not args.single_dataset:
        sub_dir += '_mix'

    # [backbone]_[pretrained datasets]_[aux.supv.]_[loop iteration]_[time]_[random number]
    log_name = sub_dir
    log_name += '_as_' if cfg.MODEL.AUX_SUPV_ON else '_'
    log_name += 'lp' + str(cfg.MODEL.N_ITER)

    if cfg.MODEL.N_ITER > 0:
        log_name += '_mlp'
        log_name += '-'.join(str(i) for i in cfg.MODEL.MLP_DIM)

    log_name += '_' + timestamp  #pama_res50_as_lp3_mlp-4_time
