...
from .imutils import cam_crop2full
...
def full_projection(pred_joints, pred_camera, center, scale, orig_shape, focal_length, retain_z=False):
    pred_cam_t_full = cam_crop2full(pred_camera, center, scale, orig_shape, focal_length[:, 0])
    batch_size = pred_joints.shape[0]
    camera_center = torch.zeros(batch_size, 2)
    camera_center[:, 0] = orig_shape[:, 1] / 2.
    camera_center[:, 0] = orig_shape[:, 0] / 2.
    pred_keypoints_full_2d = perspective_projection(pred_joints,
                                            ...
                                            camera_center=camera_center,
                                            retain_z=retain_z)
    pred_keypoints_full_2d[:, :, 0] = 2. * pred_keypoints_full_2d[:, :, 0] / orig_shape[:, 1].unsqueeze(1) - 1
    pred_keypoints_full_2d[:, :, 1] = 2. * pred_keypoints_full_2d[:, :, 1] / orig_shape[:, 0].unsqueeze(1) - 1
