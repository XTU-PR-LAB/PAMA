...
from models import knowledge_net
...
from utils.imutils import cam_crop2full, j2d_processing
...
class Trainer(BaseTrainer):
    ...
    def init_fn(self):
        ...
        if self.options.regressor == 'hmr':
        ...
        elif self.options.regressor == 'knowledge_net':
            # our model
            ...
            self.smpl_female = SMPL(model_path=path_config.SMPL_MODEL_DIR,
                                    gender='female',
                                    create_transl=False).to(self.device)
            self.model = knowledge_net(path_config.SMPL_MEAN_PARAMS, pretrained=True)
        ...
        self.criterion_head = nn.MSELoss().to(self.device)
        ...
        self.train_data_loader = DataLoader(
            self.train_ds, 
            batch_size=self.options.batch_size,  #64
            num_workers=self.options.workers,  #16
        ...
        self.valid_loader = DataLoader(
            ...
            batch_size=cfg.TEST.BATCH_SIZE,  #32
            shuffle=False,
            num_workers=cfg.TRAIN.NUM_WORKERS,  #16
            pin_memory=cfg.TRAIN.PIN_MEMORY,  #True
            sampler=val_sampler,  #None
            prefetch_factor=4
        )
    ...
    def keypoint_3d_loss(self, pred_keypoints_3d, gt_keypoints_3d, has_pose_3d):
        ...
        if len(gt_keypoints_3d) > 0:
            ...
            return torch.FloatTensor(1).fill_(0.).to(self.device)  #here,tensor([0.])
    ...
    def train(self, epoch):
        ...
        # Learning rate decay
        if self.decay_epochs_ind < len(cfg.SOLVER.EPOCHS) and epoch == cfg.SOLVER.EPOCHS[self.decay_epochs_ind]:  #False
            lr = self.optimizer.param_groups[0]['lr']
            lr_new = lr * cfg.SOLVER.GAMMA  #lr*0.1
        ...
        for step, batch in enumerate(self.train_data_loader, self.checkpoint_batch_idx):  #一个batch有64张图
            if self.options.rank == 0:
                pbar.update(1)
    ...
    def train_step(self, input_batch):
        ...
        orig_shape = input_batch['orig_shape'] #(B,2)
        focal_length = input_batch['focal_length'] #(B,)
        center = input_batch['center'] #(B,2)
        scale = input_batch['scale'] #(B,)
        gt_keypoints_orig = input_batch['keypoints_orig'] #(B,49,3)
        bbox_info = input_batch['bbox_info']


        # Get GT vertices and model joints
        # Note that gt_model_joints is different from gt_joints as it comes from SMPL
        gt_out = self.smpl(betas=gt_betas, body_pose=gt_pose[:,3:], global_orient=gt_pose[:,:3])  #在SMPL_NEUTRAL.pkl基础上使用真实θ和β计算vertices和joints等
        ...
        opt_vertices = opt_output.vertices  #(B,V,3),i.e.(B,13776,3)
        opt_joints = opt_output.joints      #(B,49,3)
        opt_head = opt_output.joints_J19[:, -6, :].unsqueeze(1)  #(B,1,3)
        ...
        img_size = orig_shape.clone().rot90().T.unsqueeze(1)
        '''gt_keypoints_orig[:, :, 0] = 2. * gt_keypoints_orig[:, :, 0] / orig_shape[:, 1].unsqueeze(1) - 1
        gt_keypoints_orig[:, :, 1] = 2. * gt_keypoints_orig[:, :, 1] / orig_shape[:, 0].unsqueeze(1) - 1'''
        # gt_keypoints_orig[:, :, :2] = 2. * (gt_keypoints_orig[:, :, :2] / img_size) - 1
        ...
        opt_cam_t = estimate_translation(opt_joints, gt_keypoints_2d_orig, focal_length=self.focal_length, img_size=self.options.img_res)  #(B,3)
        
        # opt_cam_t = estimate_translation(opt_joints, gt_keypoints_orig, focal_length=self.focal_length, img_size=self.options.img_res)
        ...
        if self.options.regressor == 'hmr':
            ...
            preds_dict, _ = self.model(images, bbox=bbox_info)
        ...
        len_loop = len(preds_dict['smpl_out']) if self.options.regressor == 'knowledge_net' else 1

        for l_i in range(len_loop):

            if self.options.regressor == 'knowledge_net':
                if l_i == 0:
            ...
            pred_output = self.smpl(betas=pred_betas, body_pose=pred_rotmat[:,1:],
                                    global_orient=pred_rotmat[:,0].unsqueeze(1), pose2rot=False)  #在静默姿势上通过LBS使用预测的θ和β计算vertices和joints等
            ...
            pred_cam_full = cam_crop2full(pred_camera, center, scale, orig_shape, focal_length)
            ...
            pred_keypoints_full_2d = perspective_projection(pred_joints,
                                                    rotation=torch.eye(3, device=self.device).unsqueeze(0).expand(batch_size, -1, -1),
                                                    translation=pred_cam_full,
                                                    focal_length=focal_length,
                                                    camera_center=torch.zeros(batch_size, 2, device=self.device))
            pred_keypoints_full_2d[:, :, :2] = pred_keypoints_full_2d[:, :, :2] / (img_size / 2.)
            ...
            loss_dict['loss_regr_pose_{}'.format(l_i)] = loss_regr_pose  #(1,)
            loss_dict['loss_regr_betas_{}'.format(l_i)] = loss_regr_betas  #(1,)

            # Compute 2D reprojection loss for the keypoints
            if cfg.LOSS.KP_2D_W > 0:
                loss_keypoints = self.keypoint_loss(pred_keypoints_full_2d, gt_keypoints_orig,
                                                    self.options.openpose_train_weight,
                                                    self.options.gt_train_weight) * cfg.LOSS.KP_2D_W
                loss_dict['loss_keypoints_{}'.format(l_i)] = loss_keypoints  #(1,)

            # Compute 3D keypoint loss
            loss_keypoints_3d = self.keypoint_3d_loss(pred_joints, gt_joints, has_pose_3d) * cfg.LOSS.KP_3D_W
            loss_dict['loss_keypoints_3d_{}'.format(l_i)] = loss_keypoints_3d  #(1,),data:tensor([0.])

            # Per-vertex loss for the shape
            if cfg.LOSS.VERT_W > 0:  #False
    ...
    def validate(self):
        ...
        J_regressor = torch.from_numpy(np.load(path_config.JOINT_REGRESSOR_H36M)).float()  #(17,6890)
        ...
        for i, target in enumerate(self.valid_loader):
            ...
            if 'h36m' in self.options.eval_dataset:
                ...
                gt_keypoints_3d = gt_keypoints_3d[:, joint_mapper_h36m, :]  #(b,14,3)
