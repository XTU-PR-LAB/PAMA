...
def main(gpu, ngpus_per_node, options):
    parse_args_extend(options)  #add options.distributed=False

    options.batch_size = cfg.TRAIN.BATCH_SIZE  #64
    options.workers = cfg.TRAIN.NUM_WORKERS  #16

    options.gpu = gpu  #None
    options.ngpus_per_node = ngpus_per_node  #1
...
if __name__ == '__main__':
    options = TrainOptions().parse_args()  #train_options.py
    parse_args_extend(options)  #update options according to config.yaml.And TRAIN.BATCH_SIZE=64
