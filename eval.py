...
parser.add_argument('--regressor', type=str, choices=['hmr', 'knowledge_net'], default='knowledge_net', help='Name of the SMPL regressor.')
parser.add_argument('--cfg_file', type=str, default='configs/config.yaml', help='config file path for network.')
...
def run_evaluation(model, dataset):
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
        ...
        with torch.no_grad():
            ...
            elif args.regressor == 'knowledge_net':
                preds_dict, _ = model(images, bbox_info)
            ...
            '''pred_cam_full = cam_crop2full(pred_camera, center, scale, full_img_shape, focal_length) #2021.1.7
...
if __name__ == '__main__':
    ...
    if args.regressor == 'knowledge_net':
        model = knowledge_net(path_config.SMPL_MEAN_PARAMS, pretrained=False)
    ...
    # dataset = MocapDataset(args, args.dataset, is_train=False)  
