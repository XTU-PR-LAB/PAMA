...
def process_image(img_file, input_res=224):
    ...
    focal_length = np.array((width * width + height * height) ** 0.5)
    bbox_info = np.float32(np.stack([cx - width / 2., cy - height / 2., b]))
    bbox_info[:2] = bbox_info[:2] / focal_length * 2.8  # [-1, 1]
    bbox_info[2] = (bbox_info[2] - 0.24 * focal_length) / (0.06 * focal_length)  # [-1, 1]
    bbox_info = torch.from_numpy(bbox_info).unsqueeze(0)
    ...
    return img_np, img, norm_img, bbox_info
...
def run_image_demo(args):
    ...
    elif args.regressor == 'prior_net':
        model = knowledge_net(path_config.SMPL_MEAN_PARAMS, pretrained=True).to(device)
    ...
    img_np, img, norm_img, bbox_info = process_image(args.img_file, input_res=constants.IMG_RES)
    with torch.no_grad():
        if args.regressor == 'hmr-spin':
            ...
            preds_dict, _ = model(norm_img.to(device), bbox_info.to(device))
    ...
    cv2.imwrite(save_name + '.png', img_np)
    cv2.imwrite(save_name + '_smpl.png', img_shape)
    cv2.imwrite(save_name + '_smpl_bg.png', img_shape_bg)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_file', type=str, default='examples/COCO_val2014_000000019667.jpg',
                        help='Path to a single input image')
    ...
    parser.add_argument('--regressor', type=str, default='prior_net', 
                        help='Name of the SMPL regressor.')
    parser.add_argument('--cfg_file', type=str, default='configs/config.yaml',
                        help='config file path.')
    '''parser.add_argument('--checkpoint', default=None,
                        help='Path to network checkpoint')'''
    parser.add_argument('--checkpoint', default='data/pretrained_model/model_checkpoint.pt', 
                        help='Path to network checkpoint')
